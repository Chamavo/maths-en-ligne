...
const StudentManagement: React.FC = () => {
  ...
  // Get current level from localStorage for a student
  const getStudentLevel = (studentName: string): number => {
  ...
  const updateStudentLevel = (studentName: string, newLevel: number) => {
    const key = `studentProgress_${studentName.toLowerCase()}`;
    const data = localStorage.getItem(key);
    let progress;
  ...
  const handleRestoreLevel = () => {
    if (!selectedStudent) return;
    
    const studentName = selectedStudent.display_name || selectedStudent.first_name;
    const levelNum = parseFloat(selectedLevel);
    
    updateStudentLevel(studentName, levelNum);
    
    toast({
      title: "Niveau restauré !",
      description: `${studentName} est maintenant au ${getLevelInfo(levelNum).name}`,
    });
