...
import { LogOut, CheckCircle, XCircle, Clock, Target, RotateCcw, ChevronRight, GripVertical, Unlock } from 'lucide-react';
...
const RevisionQuiz: React.FC<RevisionQuizProps> = ({
  ...
  useEffect(() => {
    const blockingInfo = getBlockingInfo(username);
    if (blockingInfo && blockingInfo.isBlocked) {
      setBlockedLevel(blockingInfo.level);
      setUnlockStreak(blockingInfo.currentCorrectStreak);
    }
  }, [username]);
  ...
  const submitAnswer = useCallback(() => {
    ...
    if (isCorrect) {
      ...
      if (blockedLevel !== null) {
        const { newStreak, isUnlocked, unlockedLevel: level } = incrementCorrectStreak(username);
      ...
      if (blockedLevel !== null) {
        resetCorrectStreak(username);
        setUnlockStreak(0);
      }
    }

    // Delay before next question (plus long si message de déblocage)
    const delay = isCorrect ? 1000 : 3000;
    setTimeout(() => {
      goToNextQuestion();
    }, delay);
  }, [currentExercise, selectedChoice, orderedItems, userAnswer, results, username, blockedLevel]);
      ...
      <div className="max-w-3xl mx-auto">
                  ...
                  {[...Array(10)].map((_, i) => (
                    <div
                      ...
                      className={`w-3 h-3 rounded-full ${
                        i < unlockStreak ? 'bg-green-500' : 'bg-muted'
                      }`}
                    />
                  ))}
                </div>
        ...
        >
          <div className="bg-gradient-to-r from-primary/10 to-secondary/10 rounded-xl p-6 mb-6">
            <h2 className={`font-bold text-center text-foreground ${
              currentExercise.question.length > 80 ? 'text-lg' : 'text-xl'
            }`}>
