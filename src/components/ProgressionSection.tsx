...
const ProgressionSection: React.FC<ProgressionSectionProps> = ({
  ...
  useEffect(() => {
    const initBlockingInfo = async () => {
      if (session && !session.isTeacher) {
        ...
        // Si checkAndRecordAbandon retourne null (pas d'abandon), on fetch l'info actuelle
        if (updatedInfo) {
          setBlockingInfo(updatedInfo);
        } else {
          const currentInfo = await getBlockingInfo(session.username);
          setBlockingInfo(currentInfo);
        }
      }
    };
    initBlockingInfo();
  ...
  const finishLevel = async () => {
    ...
    // Marquer le niveau comme terminé (plus en cours) et gérer réussite/échec via DB
    if (session && !session.isTeacher && currentLevel !== 0) {
      await markLevelCompleted(session.username, currentLevel); // Ensure this is awaited

      if (passed) {
        // Succès : enregistrer et supprimer les infos de blocage via DB
          ...
          <div className="bg-card rounded-2xl shadow-2xl p-8">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                {currentLevel !== 0 && (
                  <div className={`flex items-center gap-2 px-3 py-1 rounded-full ${currentErrors > getMaxErrorsForLevel(currentLevel) ? 'bg-destructive/20 text-destructive' :
                ...
                <div className={`flex items-center gap-2 ${remainingTime < 60 ? 'text-destructive' : remainingTime < 180 ? 'text-orange-600' : 'text-muted-foreground'
                  }`}>
            ...
            <div className="bg-gradient-to-r from-primary/10 to-secondary/10 rounded-xl p-8 mb-6">
              <h3 className={`font-bold text-center text-foreground mb-6 ${currentExercise.question.length > 80
              ...
              {showResult && (
                <div className={`text-center mb-4 p-4 rounded-lg ${lastAnswerCorrect ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                  }`}>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      {currentExercise.choices.map((choice, index) => (
                        <button
                          ...
                          className={`p-4 rounded-xl text-xl font-bold transition-all ${selectedChoice === choice
